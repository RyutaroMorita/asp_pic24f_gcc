 ...
 */
#define EXT_IPM(iipm)	(-CAST(PRI, (iipm) >> 5))		/* 外部表現に変換 */
#define INT_IPM(ipm)	(CAST(uint16_t, -(ipm)) << 5)	/* 内部表現に変換 */
...
{
	set_sr((current_sr() & ~0x00E0U) | iipm);
}
